...
- A tiled, semi-transparent watermark (`© Dr. S. K. Jain · <today's date>`) is rendered over every page, so it's captured in any screenshot or screen recording. The date updates automatically to the viewer's current date.
...
To change the watermark name, edit `WATERMARK_NAME` near the top of `assets/js/app.js`.
